...
${NORMAL}"
    ...
    if ! _is_yes "cr localsite from file file://'$1' in file://$(pwd) ?"; then
        ...
        if _is_yes "wget -r -k -p -E -np --user-agent=\"Mozilla/5.0 \(X11; Linux x86_64\) AppleWebKit/537.36 \(KHTML, like Gecko\) Chrome/114.0.0.0 YaBrowser/23.7.5.717 Yowser/2.5 Safari/537.36 --input-file=$1"; then

            wget -r -k -p -E -np --user-agent="Mozilla/5.0 (Linux; arm_64; Android 14; SM-G965F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.6261.119 YaBrowser/21.3.4.59 Mobile Safari/537.36" --input-file="$1"
