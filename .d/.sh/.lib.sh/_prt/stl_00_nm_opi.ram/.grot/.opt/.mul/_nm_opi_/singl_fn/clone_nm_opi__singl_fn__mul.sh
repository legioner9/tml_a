...
() {
    ...
    echo -e "${CYAN}--- () file://${PLT_PATH}/.d/.mul/nm_opi_/singl_fn/.sh ---${NORMAL}" #sistem info mesage
    ...
    cd ${dir_repo} || {
        plt_info "in () : FAIL_EXEC : 'cd file://${dir_repo}' : return 1"
        return 1
    }
    ...
    if ! [ -d "${REPO_PATH}/${name_repo}" ]; then
        if ! git clone "${clone_abs_adr}" "${name_repo}"; then
            plt_pause "in () : FAIL_EXEC : 'git clone ${clone_abs_adr}' : return 1"
            return 1
        fi
    else
