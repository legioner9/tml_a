...
MAIN: ${FNN} :: git treat anum \$1 version procedures for \$2 dir
TAGS:
ARGS: [\$1 : ] 
...
${NORMAL}"
    ...
    [[ -n "$2" ]] || {
        l_00_echo_ret1 "'$FNN() $*' in file://${fn_sh_file} , line=${LINENO} :: EMPTY_ARG '\$2' return 1"
        cd "$PPWD" || echo -e "${ECHO_WARN}in fs= file://${fn_sh_file} , line=${LINENO} , EXEC_FAIL : 'cd $PPWD' : continue${NRM}"
        return 1
    }

    l_01_is_od "$1" || {
        l_00_echo_ret1 "'$FNN() $*' in file://${fn_sh_file} , line=${LINENO} :: NOT_NUMBER '${1}' where '\$1=$1' return 1"
        cd "$PPWD" || echo -e "${ECHO_WARN}in fs= file://${fn_sh_file} , line=${LINENO} , EXEC_FAIL : 'cd $PPWD' : continue${NRM}"
        return 1
    }

    [[ -d "$2" ]] || {
        l_00_echo_ret1 "'$FNN() $*' in file://${fn_sh_file} , line=${LINENO} :: NOT_DIR 'file://${2}' where '\$3=$3' return 1"
        cd "$PPWD" || echo -e "${ECHO_WARN}in fs= file://${fn_sh_file} , line=${LINENO} , EXEC_FAIL : 'cd $PPWD' : continue${NRM}"
        return 1
    }
    ...
    local prv_dta_var_nm=$(l_02_dd2e ${d_data_ram}/.grot/.opt/.var | l_03_ibu "$1")
